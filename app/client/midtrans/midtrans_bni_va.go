...
func NewBniVa(midtrans *Midtrans) *BniVa {
	return &BniVa{
		midtrans: midtrans,
	}
}

func (b *BniVa) Pay(ctx context.Context, req any) (map[string]interface{}, error) {
...
func (b *BniVa) ClientResponse(channel *models.Channel, payment *models.Payments) (*web.PaymentResponse, error) {
	var bniVaResponse BNIVAResponse

	err := json.Unmarshal(payment.GatewayResponse, &bniVaResponse)
	if err != nil {
		return nil, err
	}

	if bniVaResponse.StatusCode != "201" {
		return nil, helpers.NewErrorTrace(fmt.Errorf(bniVaResponse.StatusMessage), b.midtrans.serviceName).WithStatusCode(http.StatusPaymentRequired)
	}
	...
	return &web.PaymentResponse{
		...
		PaymentDetail: web.PaymentDetail{
			Bank:            enums.Channel(bniVaResponse.VaNumbers[0].Bank),
			Url:             nil,
			VaNumber:        bniVaResponse.VaNumbers[0].VaNumber,
			TransactionTime: bniVaResponse.TransactionTime,
			ExpireTime:      bniVaResponse.ExpiryTime,
...
func (b *BniVa) createChargeRequest(req PaymentRequest) map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"bank_transfer": map[string]interface{}{
			"bank": "bni",
			//"va_number": "111111", -> ini dapet dari midtrans
		},
	}
}

// BNI VA JSON Request Format
/*
