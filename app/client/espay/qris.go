...
func NewQRIS(espay *Espay) *QRIS {
	return &QRIS{
		Espay: espay,
	}
}

func (c *QRIS) Pay(ctx context.Context, req any) (map[string]interface{}, error) {
	timeNow := time.Now()
	...
	chargeReq := c.createChargeQRISRequest(paymentReq, timeNow, paymentReq)
	...
	result, err := c.Espay.SendPaymentQRIS(ctx, chargeReq, asymmetricSignatureGenerate)
	if err != nil {
...
func (c *QRIS) ClientResponse(payment *models.Payments, channel *models.Channel) (*web.PaymentResponse, error) {
	...
	if qrisResponse.ResponseCode != "2005400" {
		return nil, errors.New(qrisResponse.ResponseMessage)
	}
...
func (c *QRIS) createChargeQRISRequest(req PaymentRequest, timeNow time.Time, paymentReq PaymentRequest) QRISRequest {
	return QRISRequest{
